...
class DualModelAgentConfig(I18nMixin, BaseModel):
    """Configuration for the dual model agent with fallback support."""
    ...
    primary_llm_provider: Literal[
        ...
        "groq_llm",
        "mistral_llm",
    ] = Field(..., alias="primary_llm_provider")
    ...
    fallback_llm_provider: Literal[
        ...
        "groq_llm",
        "mistral_llm",
    ] = Field(..., alias="fallback_llm_provider")

    faster_first_response: Optional[bool] = Field(True, alias="faster_first_response")
    segment_method: Literal["regex", "pysbd"] = Field("pysbd", alias="segment_method")
    use_mcpp: Optional[bool] = Field(False, alias="use_mcpp")
    mcp_enabled_servers: Optional[List[str]] = Field([], alias="mcp_enabled_servers")
    quality_threshold: Optional[float] = Field(0.4, alias="quality_threshold")
    enable_fallback: Optional[bool] = Field(True, alias="enable_fallback")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "primary_llm_provider": Description(
            en="Primary LLM provider (small model for fast response)",
            zh="主LLM提供商（小模型，用于快速响应）",
        ),
        "fallback_llm_provider": Description(
            en="Fallback LLM provider (large model for quality response)",
            zh="备用LLM提供商（大模型，用于高质量响应）",
        ),
        "faster_first_response": Description(
            en="Whether to respond as soon as encountering a comma in the first sentence to reduce latency (default: True)",
            zh="是否在第一句回应时遇上逗号就直接生成音频以减少首句延迟（默认：True）",
        ...
        "use_mcpp": Description(
            en="Whether to use MCP (Model Context Protocol) for the agent (default: False)",
            zh="是否使用为智能体启用 MCP (Model Context Protocol) Plus（默认：False）",
        ),
        "mcp_enabled_servers": Description(
            en="List of MCP servers to enable for the agent",
        ...
        "quality_threshold": Description(
            en="Quality threshold for triggering fallback (0.0-1.0, default: 0.4)",
            zh="触发回退的质量阈值（0.0-1.0，默认：0.4）",
        ),
        "enable_fallback": Description(
            en="Whether to enable fallback to large model (default: True)",
            zh="是否启用大模型回退（默认：True）",
        ),
    }
...
class AgentSettings(I18nMixin, BaseModel):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "dual_model_agent": Description(
            en="Configuration for dual model agent with fallback support", zh="双模型代理配置（支持回退）"
        ),
...
class AgentConfig(I18nMixin, BaseModel):
    ...
    conversation_agent_choice: Literal[
        "basic_memory_agent", "dual_model_agent", "mem0_agent", "hume_ai_agent", "letta_agent"
