...
class IndexTTSConfig(I18nMixin):
    ...
    api_url: str = Field("http://localhost:8000/tts", alias="api_url")
    voice_path: str = Field("examples/voice_10.wav", alias="voice_path")
    model_path: str = Field("checkpoints", alias="model_path")
    config_path: str = Field("checkpoints/config.yaml", alias="config_path")
    use_emo_text: bool = Field(True, alias="use_emo_text")
    emo_alpha: float = Field(0.6, alias="emo_alpha")
    use_random: bool = Field(False, alias="use_random")
    file_extension: Literal["wav", "mp3"] = Field("wav", alias="file_extension")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "api_url": Description(
            en="IndexTTS API endpoint URL", zh="IndexTTS API 端点 URL"
        ),
        "voice_path": Description(
            en="Path to reference voice file for voice cloning", zh="用于语音克隆的参考语音文件路径"
        ),
        "model_path": Description(
            en="Directory containing IndexTTS models", zh="包含 IndexTTS 模型的目录"
        ),
        "config_path": Description(
            en="Path to IndexTTS configuration file", zh="IndexTTS 配置文件路径"
        ),
        "use_emo_text": Description(
            en="Enable emotion extraction from text", zh="启用从文本中提取情感"
        ),
        "emo_alpha": Description(
            en="Emotion intensity (0.0-1.0)", zh="情感强度 (0.0-1.0)"
        ),
        "use_random": Description(
            en="Enable randomness in generation", zh="在生成中启用随机性"
        ),
        "file_extension": Description(
...
class TTSConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "index_tts": Description(
            en="Configuration for IndexTTS", zh="IndexTTS 配置"
        ),
    }
